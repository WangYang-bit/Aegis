...
def setup_train(hypes, filename):
    """
    ...
    folder_name = filename + current_time.strftime("%m_%d_%H_%M")
...
def create_model(hypes):
    ...
    backbone_config["postprocess"] = hypes["postprocess"]
...
def to_device(inputs, device):
    if isinstance(inputs, list):
        ...
        if (
            ...
            or isinstance(inputs, numpy.ndarray)
        ):
            return inputs
        return inputs.to(device)


def logging(loss, epoch, batch_id, batch_len, writer, pbar=None):
    """
    ...
    if pbar is None:
        print(
            "[epoch %d][%d/%d], || Loss: %.4f " % (epoch, batch_id + 1, batch_len, loss)
        )
    else:
    ...
    writer.add_scalar("Attack_loss", loss, epoch * batch_len + batch_id)
