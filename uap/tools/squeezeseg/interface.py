...
from third_party.SqueezeSegV3.src.tasks.semantic.modules.segmentator import *
from third_party.SqueezeSegV3.src.tasks.semantic.postproc.KNN import KNN
from third_party.SqueezeSegV3.src.common.laserscan import LaserScan

from uap.config import class_id_inv_map, squeeze_root
from uap.tools.cluster import get_clusters
from uap.tools.ground_detection import get_ground_plane

class SqueezeSegInterface():
    def __init__(self,device=None):
        self.modeldir = os.path.join(squeeze_root, "SqueezeSegV3")  
    ...
    def run(self, lidar):
        ...
        with torch.no_grad():
            ...
            proj_argmax = proj_output[0].max(dim=0).indices
            proj_max_values = proj_output[0].max(dim=0).values
            # count_ones = (proj_argmax == 1).sum().item()
            # print(f"Number of 1s in proj_argmax: {count_ones}")
            ...
            if self.post:
                ...
                unproj_max_values = proj_max_values[p_y, p_x]
            ...
            value_np = unproj_max_values.cpu().numpy()
            value_np = value_np.reshape((-1)).astype(np.float32)
