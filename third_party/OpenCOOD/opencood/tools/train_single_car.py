...
train_path = "/root/autodl-tmp/OPV2V/train"
test_path = "/root/autodl-tmp/OPV2V/test"
...
def main():
    opt = train_parser()
    model_root = "/home/UAP_attack/models/OpenCOOD"
    model_dir = os.path.join(model_root, "pointpillar_single_car_large/config.yaml")
    hypes = yaml_utils.load_yaml(model_dir, None)

    model_hypes = yaml_utils.load_yaml(opt.hypes_yaml, opt)
    hypes["root_dir"] = train_path 
    hypes["validate_dir"] = test_path
    multi_gpu_utils.init_distributed_mode(opt)

    print('-----------------Dataset Building------------------')
    opencood_train_dataset = build_dataset(hypes, visualize=False, train=False)
    ...
    if opt.init_model_dir:
        ...
        epoch, init_model = train_utils.load_saved_model(opt.init_model_dir,
                                                         init_model)
        ...
        shrink_conv_params = init_model.shrink_conv.state_dict()
        model.shrink_conv.load_state_dict(shrink_conv_params)
        model.shrink_conv.requires_grad_(False)
