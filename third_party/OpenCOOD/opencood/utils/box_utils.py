...
def corner_to_center_torch(corner3d, order='lwh'):
    ...
    box3d : Tensor
        (N, 7)
    """
    assert corner3d.dim() == 3, "corner3d 必须是 3D tensor"
    batch_size = corner3d.shape[0]

    xyz = torch.mean(corner3d[:, [0, 3, 5, 6], :], dim=1)

    h = torch.abs(torch.mean(corner3d[:, 4:, 2] - corner3d[:, :4, 2], dim=1, keepdim=True))

    l = (torch.sqrt(torch.sum((corner3d[:, 0, :2] - corner3d[:, 3, :2]) ** 2, dim=1, keepdim=True)) +
         torch.sqrt(torch.sum((corner3d[:, 2, :2] - corner3d[:, 1, :2]) ** 2, dim=1, keepdim=True)) +
         torch.sqrt(torch.sum((corner3d[:, 4, :2] - corner3d[:, 7, :2]) ** 2, dim=1, keepdim=True)) +
         torch.sqrt(torch.sum((corner3d[:, 5, :2] - corner3d[:, 6, :2]) ** 2, dim=1, keepdim=True))) / 4

    w = (torch.sqrt(torch.sum((corner3d[:, 0, :2] - corner3d[:, 1, :2]) ** 2, dim=1, keepdim=True)) +
         torch.sqrt(torch.sum((corner3d[:, 2, :2] - corner3d[:, 3, :2]) ** 2, dim=1, keepdim=True)) +
         torch.sqrt(torch.sum((corner3d[:, 4, :2] - corner3d[:, 5, :2]) ** 2, dim=1, keepdim=True)) +
         torch.sqrt(torch.sum((corner3d[:, 6, :2] - corner3d[:, 7, :2]) ** 2, dim=1, keepdim=True))) / 4

    theta = (torch.atan2(corner3d[:, 1, 1] - corner3d[:, 2, 1],
                         corner3d[:, 1, 0] - corner3d[:, 2, 0]) +
             ...
             torch.atan2(corner3d[:, 4, 1] - corner3d[:, 7, 1],
                         corner3d[:, 4, 0] - corner3d[:, 7, 0]))[:, None] / 4

    if order == 'lwh':
        return torch.cat([xyz, l, w, h, theta], dim=1)
    elif order == 'hwl':
        return torch.cat([xyz, h, w, l, theta], dim=1)
    else:
        sys.exit('Unknown order')
