...
class Communication(nn.Module):
    ...
    def forward(self, batch_confidence_maps, B):
        """
        Args:
            batch_confidence_maps: [(L, pm, H, W), (L2, pm, H, W), ...]
        """
        ...
        if self.multi_scale:
            ...
            for i in range(self.num_levels):
                ...
                if len(backbone.deblocks) > 0:
                    #shape(B, 128, 96, 352)
                    new_x = backbone.deblocks[i](x_fuse)
                    ups.append(new_x)
                    # ups.append(torch.zeros_like(new_x).to(new_x.device))
                    # ups.append(torch.zeros_like(new_x).to(new_x.device))
                else:
                    ups.append(x_fuse)
