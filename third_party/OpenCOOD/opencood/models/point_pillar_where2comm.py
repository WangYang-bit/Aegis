...
class PointPillarWhere2comm(nn.Module):
    ...
    def feature_encoder(self, data_dict):
        voxel_features = data_dict['processed_lidar']['voxel_features']
        voxel_coords = data_dict['processed_lidar']['voxel_coords']
        voxel_num_points = data_dict['processed_lidar']['voxel_num_points']
        record_len = data_dict['record_len']
        ...
        # N, C, H', W': [N, 256, 48, 176]
        data_dict['spatial_features_2d'] = batch_dict['spatial_features_2d']
        data_dict['spatial_features'] = batch_dict['spatial_features']
        return data_dict
    
    def fusion_decoder(self, data_dict):
        record_len = data_dict['record_len']
        ...
        if self.multi_scale:
            # Bypass communication cost, communicate at high resolution, neither shrink nor compress
            fused_feature, communication_rates = self.fusion_net(data_dict['spatial_features'],
                                                                 psm_single,
                                                                 record_len,
                                                                 pairwise_t_matrix,
                                                                 self.backbone)
        ...
        psm = self.cls_head(fused_feature)
        rm = self.reg_head(fused_feature)
        output_dict = {'psm': psm, 'rm': rm, 'com': communication_rates,'fused_feature':fused_feature}
        return output_dict

    def decoder(self, data_dict):
        spatial_features_2d = data_dict['spatial_features_2d']
        # Down-sample feature to reduce memory
        if self.shrink_flag:
            spatial_features_2d = self.shrink_conv(spatial_features_2d)
        ...
        output_dict = {'psm': psm, 'rm': rm, 'feature':feature}
        return output_dict
