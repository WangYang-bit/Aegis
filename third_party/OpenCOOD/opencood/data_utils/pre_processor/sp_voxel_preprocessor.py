...
class SpVoxelPreprocessor(BasePreprocessor):
    ...
    def preprocess(self, pcd_np):
        """
        Preprocess point cloud data by converting it into voxel representation.

        Parameters
        ----------
        pcd_np : np.ndarray
        Input point cloud data in NumPy array format.

        Returns
        -------
        data_dict : dict
            A dictionary containing voxel features, voxel coordinates, and the 
            number of points per voxel.

        Description
        -----------
        This function preprocesses the point cloud data (pcd_np) by transforming 
        it into a voxel representation. The method used for voxelization depends 
        on the `self.spconv` attribute:
        - If `self.spconv == 1`, it uses `self.voxel_generator.generate` to generate voxels.
        - Otherwise, it uses `self.voxel_generator.point_to_voxel`.

        The function then processes the voxelization output, handling both cases 
        where the output might be a dictionary or a tuple. If `self.spconv == 2`, 
        the voxel data is converted into NumPy arrays before storing them in the 
        `data_dict`.

        The `data_dict` contains the following keys:
        - 'voxel_features': The features of the voxels.
        - 'voxel_coords': The coordinates of the voxels.
        - 'voxel_num_points': The number of points in each voxel.
        """
        data_dict = {}
        if self.spconv == 1:
