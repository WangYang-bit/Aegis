...
class VoxelPostprocessor(BasePostprocessor):
    ...
    def post_process(self, data_dict, output_dict):
        ...
        for cav_id, cav_content in data_dict.items():
            ...
            if 'transformation_matrix' in cav_content:
                ...
                transformation_matrix = torch.from_numpy(np.identity(4)).float().to(anchor_box.device)
    ...
    def post_process_single_car(self, data_dict, output_dict):
        """
        Process the outputs of the model to 2D/3D bounding box.
        Step1: convert each cav's output to bounding box format
        Step2: project the bounding boxes to ego space.
        ...
        # the transformation matrix to ego space
        if 'transformation_matrix' in data_dict:
        ...
        # Adjust mask_reg to match the shape of batch_box3d
        mask_reg = mask.unsqueeze(2).repeat(1, 1, 7)

        for i in range(mask_reg.size(0)):
            ...
            if len(boxes3d) != 0:
                ...
                # filter out the prediction out of the range.
                pre_mask = \
                    box_utils.get_mask_for_boxes_within_range_torch(projected_boxes3d)
                projected_boxes3d = projected_boxes3d[pre_mask, :, :]
                scores = scores[pre_mask]

                assert scores.shape[0] == projected_boxes3d.shape[0]

                pred_box3d_tensor.append(projected_boxes3d)
    ...
    def delta_to_boxes3d(deltas, anchors, channel_swap=True):
        ...
        if deltas.is_cuda:
            anchors = anchors.cuda(deltas.device)
            boxes3d = boxes3d.cuda(deltas.device)
