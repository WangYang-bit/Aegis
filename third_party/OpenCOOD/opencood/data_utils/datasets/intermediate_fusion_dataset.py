...
class IntermediateFusionDataset(basedataset.BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        scene_id = base_data_dict[ego_id]["scenario_id"]
        timestamp_index = base_data_dict[ego_id]["timestamp_index"]
        timestamp_id = base_data_dict[ego_id]["timestamp_id"]
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            true_ego_position = selected_cav_base["params"]["true_ego_pos"][0:3]
            relative_pose = position_project(true_ego_position, world_to_ego)
            cav_pose[cav_id] = relative_pose
        ...
        # (x, y, z, h, w, l, yaw) in ego coordinate
    ...
    def collate_batch_train(self, batch):
        ...
        for i in range(len(batch)):
            ...
            if "cav_list" in ego_dict:
                cav_list.append(ego_dict["cav_list"])
    ...
    def collate_batch_test(self, batch):
        ...
        if "scene_id" in batch[0]["ego"]:
            output_dict["ego"].update({"scene_id": batch[0]["ego"]["scene_id"]})
        if "timestamp_index" in batch[0]["ego"]:
            output_dict["ego"].update(
                {"timestamp_index": batch[0]["ego"]["timestamp_index"]}
            )
        if "timestamp_id" in batch[0]["ego"]:
            output_dict["ego"].update({"timestamp_id": batch[0]["ego"]["timestamp_id"]})
        if "origin_lidar" in output_dict["ego"]:
            output_dict["ego"].update(
                {"origin_lidar": output_dict["ego"]["origin_lidar"][0]}
            )

        if "lidar_pose" in batch[0]["ego"]:
            output_dict["ego"].update({"lidar_pose": batch[0]["ego"]["lidar_pose"]})

        if "cav_list" in batch[0]["ego"]:
            output_dict["ego"].update({"cav_list": batch[0]["ego"]["cav_list"]})
        
        if "cav_pose" in batch[0]["ego"]:
            output_dict["ego"].update({"cav_pose": batch[0]["ego"]["cav_pose"]})
